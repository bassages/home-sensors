...
class Thermometer extends AbstractSensor {
    ...
    Temperature getAmbientTemperature(final Expect expect) throws IOException, SensortagException {
        enable(expect);
    ...
    private double ambientTemperatureFromHex(final String hexValue) {
        double result = 0.0;
        final String[] hexValues = hexValue.split(" ");
        if (hexValues.length == 4) {
            final int rawAmbient = Integer.parseInt(hexValues[3] + hexValues[2], 16);
