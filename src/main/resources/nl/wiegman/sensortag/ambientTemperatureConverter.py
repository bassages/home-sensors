...
def calculateAmbientTemperature(objT, ambT):
    m_tmpAmb = ambT/128.0
    ...
    tObj = (tObj - 273.15)
#    print "Target temperature: %.2f" % tObj
    return m_tmpAmb

class AmbientTemperatureConverter:

    def fromHex(self, hex):
        rval = hex.split()
        objT = floatfromhex(rval[1] + rval[0])
        ambT = floatfromhex(rval[3] + rval[2])
        return calculateAmbientTemperature(objT, ambT)
